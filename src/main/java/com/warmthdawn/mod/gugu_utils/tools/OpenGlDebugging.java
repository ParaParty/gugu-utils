    ...
    public static void dumpAllIsEnabled()
    {
        for (int i = 0; i < instance.propertyList.length; ++i) {
            if (instance.propertyList[i].fetchCommand.equals("glIsEnabled()")) {
                GuGuUtils.logger.info(instance.propertyList[i].name + ":" +
                        GL11.glIsEnabled(instance.propertyList[i].gLconstant)
    ...
    public static void dumpAllType(String type) {
        for (int i = 0; i < instance.propertyList.length; ++i) {
            if (instance.propertyList[i].category.equals(type)) {
                GuGuUtils.logger.info(instance.propertyList[i].name + ":" + getPropertyAsString(i));
                GuGuUtils.logger.info(" (" + instance.propertyList[i].description + ")");
            }
        }
    }
